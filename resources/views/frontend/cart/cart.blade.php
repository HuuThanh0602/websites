    ...
    <div class="container">
        ...
        <div class="cart-toggle-container">
            <div class="cart-toggle-buttons">
                <a href="{{ route('cart.checkout2', ['type' => 2])  }}" class="cart-toggle-btn {{ $type == 2 ? 'active' : '' }}">
                    <i class="fas fa-shopping-cart"></i> Giỏ hàng
                </a>
                <a href="{{ route('cart.checkout2', ['type' => 3]) }}" class="cart-toggle-btn {{ $type == 3 ? 'active' : '' }}">
                    <i class="fas fa-clipboard-list"></i> Giỏ hàng Order
                </a>
            </div>
        </div>

        <!-- Cart content (không lồng form lớn ở đây) -->
            ...
            @if (!empty($carts) && count($carts) > 0)
                ...
                <div class="cart-items-table">
                    <div class="cart-header-row d-none d-md-flex">
                        ...
                        <div class="header-item col-md-2 text-center">Đơn giá</div>
                        <div class="header-item col-md-1 text-center">Giá</div>
                        <div class="header-item col-md-1 text-center">Thao tác</div>
                    </div>
                    ...
                    <div class="cart-item" >
                        <div class="row align-items-center">
                                ...
                                <div class="product-select">
                                    <label class="product-checkbox">
                                        <input type="checkbox" name="selected_products[]" 
                                            value="{{ json_encode(['product_id' => $cart->product_id ?? $cart->id, 'product_variant_id' => $cart->product_variant_id]) }}" 
                                            class="product-checkbox-input" data-cart-id="{{ $cart->id }}">
                                        <span class="checkmark"></span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="col-md-5">
                                <div class="product-info d-flex align-items-center">
                                    ...
                                    <div class="product-details">
                                        ...
                                        <!-- Product Attributes Selection -->
                                        <form action="{{ route('cart.updateAttribute') }}" method="POST" class="update-attribute-form" id="update-attribute-form-{{ $cart->id }}">
                                            @csrf
                                            <input type="hidden" name="product_id" value="{{ $cart->id }}">
                                            <input type="hidden" name="product_variant_id" id="product_variant_id_{{ $cart->id }}" value="{{ $cart->product_variant_id }}">
                                            <input type="hidden" name="type" value="{{ $type }}">

                                            <label>Size:</label>
                                            @if ($type==2)
                                            <select class="attribute-select size-select" data-form-id="{{ $cart->id }}" name="new_product_variant_id">
                                                @foreach ($cart->attribute as $attribute)
                                                    <option value="{{ $attribute->product_variant_id }}" 
                                                            {{ $cart->product_variant_id == $attribute->product_variant_id ? 'selected' : '' }} 
                                                            {{ $attribute->quantity <= 0 ? 'disabled' : '' }}>
                                                        {{ $attribute->name }} (Còn lại: {{ $attribute->quantity }})
                                                    </option>
                                                @endforeach
                                            </select>
                                            @else
                                        ...
                                        <div class="product-inventory">
                                            ...
                                            <span class="inventory-status {{ ($cart->stock ?? 10) > 5 ? 'in-stock' : 'low-stock' }}">
                                                Còn lại: {{ $cart->quantity }} sản phẩm
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-2">
                                ...
                                {{ convert_price($cart->price ,true) }}đ
                ...
                <!-- Cart Summary (form riêng cho checkout) -->
                <form action="{{ route('cart.checkout3', ['type' => 2]) }}" method="GET" id="cart-form">
                    @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.size-select').forEach(function(select) {
        select.addEventListener('change', function() {
            ...
            let form = document.getElementById('update-attribute-form-' + formId);
            let input = document.getElementById('product_variant_id_' + formId);

            if (form && input) {
                // Cập nhật input ẩn
                form.submit(); // Gửi form tự động
            } else {
                console.error('Form hoặc input không tồn tại cho formId: ' + formId);
            }
        });
    });
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', function(e) {
            const selectedProducts = document.querySelectorAll('.product-checkbox-input:checked');
            ...
            // Thêm các giá trị checkbox vào form trước khi submit
            selectedProducts.forEach((checkbox, index) => {
                const input = document.createElement('input');
                input.type = 'hidden';
                input.name = 'selected_products[' + index + ']';
